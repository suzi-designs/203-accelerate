<?php
/*
Plugin Name: Good Old Twitter Feed Widget
Plugin URI: http://www.whiletrue.it/
Description: Displays your public Twitter messages in the sidebar of your blog. All your visitors will see your public tweets!
Author: whiletrue
Version: 1.2.1
...
/**
 * GoodOldTwitterWidget Class
 */
class GoodOldTwitterWidget extends WP_Widget
{
    /** @type {string} */ private $languagePath;
    ...
    public function __construct()
    {
        ...
        $this->options = array(
            ...
            array(
              ...
              <button class="button" style="margin-left:20px;" onclick="javascript:jQuery(\'.gotw_advanced\').toggle(); return false;">' . __('Click to show / hide', 'rstw') . '</button>
            </h2>
            <div class="gotw_advanced" style="display:none;">',
                'type'  => 'separator'
            ),
            array(
            ...
            array(
                'name'  => 'thumbnail',  'label'  => __('Include profile thumbnail before tweets', 'rstw'),
                'type'  => 'checkbox',  'default' => false
            ),
            array(
            ...
            array(
                'name'  => 'images',  'label'  => __('Show tweet images', 'rstw'),
                'type'  => 'checkbox',  'default' => true
            ),
            array(
        ...
        parent::__construct(false, 'Good Old Twitter', array(), $control_ops);
    }
    ...
    public function widget($args, $instance)
    {
        ...
        if ($title != '') {
            ...
            if (isset($instance['title_thumbnail']) && $instance['title_thumbnail']) {
                ...
                if ($twitter_thumb == '') {
                    ...
                    if (!$this->cb) {
                        $this->good_old_twitter_codebird_set($instance);
                    }
                    $user_data =  $this->cb->users_show(array('screen_name' => $instance['username']));
                    $twitter_thumb = $user_data['profile_image_url_https'];
                    set_transient($transient_name, $twitter_thumb, 60 * 60 * 24); // 1 day
                }
                if ($twitter_thumb != '') {
                    $title_thumb = '<img src="' . $twitter_thumb . '" alt="' . $title . '" title="' . $title . '" class="good_old_twitter_author" /> ';
                }
            }
        ...
        echo $this->good_old_twitter_messages($instance);
        echo $after_widget;
    }
    ...
    public function form($instance)
    {
        ...
        echo '<div class="gotw_form">';

        foreach ($this->options as $val) {
            if ($val['type'] == 'separator') {
                if (isset($val['label']) && $val['label'] != '') {
                    echo '<h3>' . $val['label'] . '</h3>';
                } elseif (isset($val['code']) && $val['code'] != '') {
                    echo $val['code'];
                } else {
                    echo '<hr />';
                }
            ...
            } elseif (isset($val['type']) && ($val['type'] == 'text' || $val['type'] == 'password')) {
                echo '
          ...
          <div class="gotw_clear"></div>';
            } elseif (isset($val['type']) && $val['type'] == 'checkbox') {
                $checked = (isset($instance[$val['name']]) && $instance[$val['name']]) ? 'checked="checked"' : '';
                echo '
          <div class="gotw_checkbox"><input id="' . $this->get_field_id($val['name']) . '" name="' . $this->get_field_name($val['name']) . '" type="checkbox" ' . $checked . ' /></div>
          <label for="' . $this->get_field_id($val['name']) . '">' . $val['label'] . '</label>
          <div class="gotw_clear"></div>';
            } elseif (isset($val['type']) && $val['type'] == 'donate') {
      ...
      <style>
      .gotw_form h2, .gotw_form h3, .gotw_form .description { text-align:center; margin-top:1em; margin-bottom:0.5em; }
      .gotw_form input[type="text"], .gotw_form input[type="password"] { float:left; width:200px; }
      .gotw_form .gotw_checkbox { float:left; width:200px; text-align:right; }
      .gotw_form label { width:270px; padding-left:5px; }
      .gotw_form .gotw_clear { clear:both; height:2px; margin-bottom:1px; border-bottom:1px solid #eee; }
      </style>';
    }
    ...
    public function good_old_twitter_codebird_set($options)
    {
    ...
    public function good_old_twitter_messages($options)
    {
        if ($options['debug']) {
        ...
        if (!isset($this->cb)) {
            $this->good_old_twitter_codebird_set($options);
        }
            ...
            if (empty($twitter_data) || count($twitter_data) < 1 || isset($twitter_data['errors'])) {
                ...
                if (!isset($twitter_data['errors']) && count($twitter_data) > 0) {
                    ...
                    foreach ($twitter_data as $key => $message) {
                        if (!is_array($message) || !isset($message['full_text'])) {
                            continue;
                        }
                        // four byte utf8: 11110www 10xxxxxx 10yyyyyy 10zzzzzz
                        $twitter_data[$key]['full_text'] = preg_replace('/[\xF0-\xF7][\x80-\xBF]{3}/', '', $message['full_text']);
                    }
      ...
      <ul class="good_old_twitter_widget">';
        ...
        foreach ($twitter_data as $message) {
            ...
            if (count($message['retweeted_status']) > 0) {
                $msg = 'RT @' . $message['retweeted_status']['user']['screen_name'] . ': ' . $message['retweeted_status']['full_text'];
            ...
            if (
                ...
                $out .= '<br><a href="' . $message['entities']['media'][0]['media_url_https'] . '" target="_blank">
            <img src="' . $message['entities']['media'][0]['media_url_https'] . '" style="max-width:100%" alt="" title="">
          </a>';
            }
            ...
            if ($options['update']) {
                ...
                $out .= '<span class="gotw_comma">,</span> <span class="twitter-timestamp" title="' . date(__('Y/m/d H:i', 'rstw'), $time) . '">' . $h_time . '</span>';
            }
        ...
        if ($options['link_user']) {
            $original_link_user = '<div class="gotw_link_user"><a href="http://twitter.com/' . $options['username'] . '" ' . $link_target . '>' . $options['link_user_text'] . '</a></div>';
            $out .= apply_filters('gotw_link_user', $original_link_user, $options);
        }
        ...
        <script>!function(d,s,id){var js,fjs=d.getElementsByTagName(s)[0],p=/^http:/.test(d.location)?"http":"https";if(!d.getElementById(id)){js=d.createElement(s);js.id=id;js.src=p+"://platform.twitter.com/widgets.js";fjs.parentNode.insertBefore(js,fjs);}}(document, "script", "twitter-wjs");</script>';
            $out .= apply_filters('gotw_button_follow', $original_button_follow, $options);
        }
        ...
        return apply_filters('gotw_output', $out, $options);
    }
} // class GoodOldTwitterWidget


// SHORTCODE FUNCTION
function good_old_twitter_shortcode($atts)
{
    // e.g. [good_old_twitter username="" consumer_key="" consumer_secret="" access_token="" access_token_secret=""]

    $rstw = new GoodOldTwitterWidget();
    ...
    return $rstw->good_old_twitter_messages($atts);
}


// register GoodOldTwitterWidget widget
add_action('widgets_init', create_function('', 'return register_widget("GoodOldTwitterWidget");'));

add_shortcode('good_old_twitter', 'good_old_twitter_shortcode');
